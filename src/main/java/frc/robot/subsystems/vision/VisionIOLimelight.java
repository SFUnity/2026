...
public class VisionIOLimelight implements VisionIO {
  ...
  public Translation2d getFieldRelativeSpherePosition(
      double[] detection, double ballRadius, PoseManager poseManager) {
    ...
    sphereTranslation =
        sphereTranslation
            .rotateBy(new Rotation2d(Units.degreesToRadians(xAngle)))
            .rotateBy(new Rotation2d(Units.degreesToRadians(position.get("yaw"))));
    // move by cam position
    Translation2d camPosition =
        new Translation2d(position.get("forwardOffset"), position.get("sideOffset"));
    sphereTranslation = sphereTranslation.plus(camPosition);
    // move and rotate by robot position
    sphereTranslation =
        sphereTranslation.rotateBy(poseManager.getRotation()).plus(poseManager.getTranslation());
